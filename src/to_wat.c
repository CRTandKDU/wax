...
str_t expr_to_wat(expr_t* expr, int indent, char lr){
  ...
  if (expr->key == EXPR_LET){
    ...
    if (tok->tag == TOK_IDT){
      ...
      list_add(&wat_strs, d);
      printf( "Got string %s: wat_str_ptr = %d, increment = %d\n", tok->val.data, wat_str_ptr, tok->val.len );
    ...
    str_add(&out, " (export \"");
#ifdef WASI
  if( 0 == strcmp( funcname.data, "main" ) ){
    str_add( &out, "_start" );
  }
  else{
    str_add(&out, funcname.data);
...
/* #endif */
  ...
  str_add(&out,"(module \n");
#ifdef WASI
  str_add( &out, "(import \"wasi_snapshot_preview1\" \"fd_write\" (func $fd_write (param i32 i32 i32 i32) (result i32)))\n" );
#else
  ...
  printf( "Setting $wax::min_addr. wat_str_ptr = %d, aligned = %s\n", wat_str_ptr, wsps ); 
